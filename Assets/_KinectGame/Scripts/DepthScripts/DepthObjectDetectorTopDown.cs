...
public class DepthObjectDetectorTopDown : MonoBehaviour
{
    public static DepthObjectDetectorTopDown Instance;

    public GameObject DepthSourceManager;
    public GameObject TopDownObjectPrefab;
    ...
    public ObjectCreationMode currentMode;
    ...
    public float maxAllowedMovementRadius = 10f;
    ...
    IEnumerator ExecuteEveryNFrames()
    {
        while (true)
        {
            ...
            {
                InstantiateAndUpdateObjectsWithMesh();
            }
        }
    }
    ...
    void InstantiateAndUpdateObjectsWithMesh()
    {
        ...
        foreach (var contour in contours)
        {
            ...
            if (area > minBlobSizesToRemove)
            {
                ...
                for (int i = 0; i < contour.Length; i++)
                {
                    ...
                    worldPoints[i] = new Vector3(viewportPoint.x, viewportPoint.y, 0);
                }

                worldPoints = SimplifyPolygon(worldPoints, simplificationTolerance);
                OpenCvSharp.Rect boundingBox = Cv2.BoundingRect(contour);
                Vector2 newCenter = DepthToViewport(new Vector2(boundingBox.X + boundingBox.Width / 2, boundingBox.Y + boundingBox.Height / 2));
                ...
                {
                    float distance = Vector2.Distance(kvp.Value, newCenter);
                ...
                {
                    // Update position & rotation instead of recreating
                    closestObject.transform.localPosition = new Vector3(newCenter.x, newCenter.y, 0);

                    trackedObjects[closestObject] = newCenter;
                    updatedObjects.Add(closestObject);

                    if (!Utilities.IsInSelectedLayers(layersToIgnoreMeshUpdates, closestObject))
                    {
                        UpdateSprite(closestObject, worldPoints);
                        UpdateCollision(closestObject, worldPoints);
                    }
                ...
                {
                    // Create a new object
                    GameObject obj = Instantiate(TopDownObjectPrefab, spawnedObjectsParentRoot.transform.InverseTransformPoint(newCenter), Quaternion.Euler(0, 0, 0), spawnedObjectsParentRoot);
                    ...
                    trackedObjects[obj] = newCenter;
                    updatedObjects.Add(obj);

                    UpdateSprite(obj, worldPoints);
                    UpdateCollision(obj, worldPoints);
                }
            }
        }
    ...
    private void InstantiateAndDestroyObjectsWithMesh()
    {
        ...
        foreach (var contour in contours)
        {
            ...
            if (area > minBlobSizesToRemove) // Filter out small objects
            {
                ...
                GameObject obj = Instantiate(TopDownObjectPrefab, spawnedObjectsParentRoot.transform.InverseTransformPoint(gameObjectWorldPosition), TopDownObjectPrefab.transform.rotation, spawnedObjectsParentRoot);
                obj.transform.localPosition = new Vector3(obj.transform.localPosition.x, obj.transform.localPosition.y, 0);
                obj.transform.localScale = generated2DMeshObjScale;
                _SpawnedObjects.Add(obj);

                //DEBUG CENTROID OBJ GEN (got replaced by gizmo)
                /*
                if (showDebugCenterPos)
                {
                    Vector2 centerWorldPos = new Vector3(gameObjectWorldPosition.x, gameObjectWorldPosition.y);
                    GameObject centerObj = Instantiate(DebugObjectCenterPrefab, centerWorldPos, TopDownObjectPrefab.transform.rotation, spawnedObjectsParentRoot);
    ...
    private void InstantiateObjectsWith3DMesh()
    {
        // Find contours in the binary image
        HierarchyIndex[] hierarchy;
        Point[][] contours;
        ...
        foreach (var contour in contours)
        {
            ...
            if (area > minBlobSizesToRemove) // Filter out small objects
            {
                ...
                // Create a new GameObject
                GameObject obj = Instantiate(TopDownObjectPrefab, gameObjectWorldPosition, TopDownObjectPrefab.transform.rotation, spawnedObjectsParentRoot);
                obj.transform.localPosition = new Vector3(obj.transform.localPosition.x, obj.transform.localPosition.y, 0);
                _SpawnedObjects.Add(obj);
                ...
                if (showDebugCenterPos)
                {
                    ...
                    GameObject centerObj = Instantiate(TopDownObjectPrefab, centerWorldPos, TopDownObjectPrefab.transform.rotation, spawnedObjectsParentRoot);
    ...
    private void UpdateSprite(GameObject obj, Vector2[] worldPoints)
    {
        if (!obj.TryGetComponent<SpriteRenderer>(out SpriteRenderer spriteRenderer))
            spriteRenderer = obj.AddComponent<SpriteRenderer>();
    ...
    private void UpdateCollision(GameObject obj, Vector2[] worldPoints)
    {
        if (!obj.TryGetComponent<PolygonCollider2D>(out PolygonCollider2D polygonCollider))
            polygonCollider = obj.AddComponent<PolygonCollider2D>();

        // Temporarily disable to avoid unwanted trigger events
        bool wasTrigger = polygonCollider.isTrigger;
        ...
        for (int i = 0; i < worldPoints.Length; i++)
        {
            worldPoints[i] -= centroid;
            worldPoints[i] /= generated2DMeshObjScale.x;
        }

        polygonCollider.SetPath(0, worldPoints);

        // Re-enable collider after update
        polygonCollider.enabled = true;
        polygonCollider.isTrigger = wasTrigger; // Ensure trigger state is restored
    }

    private Mesh TriangulateContour(Vector3[] points)
    {
    ...
    private Sprite CreateSpriteFromContour(Vector2[] worldPoints)
    {
    ...
    private void DrawLine(Texture2D texture, Vector2 start, Vector2 end, Color32 color)
    {
