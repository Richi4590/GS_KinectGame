...
public class DepthObjectDetectorSideScroller : MonoBehaviour
{
    public static DepthObjectDetectorSideScroller Instance;

    public GameObject DepthSourceManager;
    public GameObject SideScrollObjectPrefab;
    ...
    public bool instantiateMeshObjectsPerFrame = false;
        ...
        foreach (var contour in contours)
        {
            ...
            if (area > minBlobSizesToRemove)
            {
                ...
                if (closestObject != null && minDistance <= maxAllowedMovementRadius)
                {
                    ...
                    UpdateSpriteAndCollision(closestObject, worldPoints);
                    updatedObjects.Add(closestObject);
                }
                else
                {
                    // Create a new object
                    GameObject obj = Instantiate(SideScrollObjectPrefab, spawnedObjectsParentRoot.transform.InverseTransformPoint(newCenter), Quaternion.Euler(0, 0, 0), spawnedObjectsParentRoot);
    ...
    void InstantiateAndDestroyObjectsWithMesh()
    {
        ClearSpawnedObjects();
        ...
        foreach (var contour in contours)
        {
            ...
            if (area > minBlobSizesToRemove) // Filter out small objects
            {
                ...
                GameObject obj = Instantiate(SideScrollObjectPrefab, spawnedObjectsParentRoot.transform.InverseTransformPoint(gameObjectWorldPosition), SideScrollObjectPrefab.transform.rotation, spawnedObjectsParentRoot);
                obj.transform.localPosition = new Vector3(obj.transform.localPosition.x, obj.transform.localPosition.y, 0);
                obj.transform.localScale = generated2DMeshObjScale;
                _SpawnedObjects.Add(obj);
    ...
    void InstantiateObjectsWith3DMesh()
    {
        ...
        foreach (var contour in contours)
        {
            ...
            if (area > minBlobSizesToRemove) // Filter out small objects
            {
                ...
                // Create a new GameObject
                GameObject obj = Instantiate(SideScrollObjectPrefab, gameObjectWorldPosition, SideScrollObjectPrefab.transform.rotation, spawnedObjectsParentRoot);
                obj.transform.localPosition = new Vector3(obj.transform.localPosition.x, obj.transform.localPosition.y, 0);
                _SpawnedObjects.Add(obj);
                ...
                if (showDebugCenterPos)
                {
                    ...
                    GameObject centerObj = Instantiate(SideScrollObjectPrefab, centerWorldPos, SideScrollObjectPrefab.transform.rotation, spawnedObjectsParentRoot);
    ...
    private void UpdateSpriteAndCollision(GameObject obj, Vector2[] worldPoints)
    {
        if (!obj.TryGetComponent<SpriteRenderer>(out SpriteRenderer spriteRenderer))
            spriteRenderer = obj.AddComponent<SpriteRenderer>();
    ...
    {
        foreach (var item in trackedObjects) DestroyCustom(item.Key);
        foreach (GameObject obj in _SpawnedObjects) DestroyCustom(obj);
    ...
    private void DestroyCustom(GameObject gameObject)
    {

        MovableObject[] childrenComponent = gameObject.transform.GetComponentsInChildren<MovableObject>();

        for (int j = 0; j < childrenComponent.Length; j++)
        {
            childrenComponent[j].transform.SetParent(GameObject.Find("SpawnedObjectsRoot").transform);
        }
