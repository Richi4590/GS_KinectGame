...
namespace IndieMarc.TopDown
{
    public class PlayerCharacter : MonoBehaviour
    {
        ...
        public float running_accel = 1f;
        public float running_deccel = 1f;
        public float running_max = 1f;
        ...
        void Start()
        {
            controls = PlayerControls.Get(player_id);
        }
        ...
        void FixedUpdate()
        {
            ...
            if (controls.GetIsRunningAction())
            {
                desiredSpeedX = Mathf.Abs(move_input.x) > 0.1f ? move_input.x * running_max : 0f;
                desiredSpeedY = Mathf.Abs(move_input.y) > 0.1f ? move_input.y * running_max : 0f;

                accelerationX = Mathf.Abs(move_input.x) > 0.1f ? running_accel : running_deccel;
                accelerationY = Mathf.Abs(move_input.y) > 0.1f ? running_accel : running_deccel;
            }
            else
            {
                desiredSpeedX = Mathf.Abs(move_input.x) > 0.1f ? move_input.x * move_max : 0f;
                desiredSpeedY = Mathf.Abs(move_input.y) > 0.1f ? move_input.y * move_max : 0f;

                accelerationX = Mathf.Abs(move_input.x) > 0.1f ? move_accel : move_deccel;
                accelerationY = Mathf.Abs(move_input.y) > 0.1f ? move_accel : move_deccel;
            }
